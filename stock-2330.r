...
h=getSymbols("2330.TW",auto.assign=FALSE) #下載股市資料 getSymbols並定義一個字母h 以2330為例子
h	#抓取2330資料
chartSeries(h)	#繪製股價歷史線圖,所有資料點
chartSeries(h["2015-01::2016-03"],theme="white")	#如果我只想要看2012年1月到2016年3月的圖形,背景換白色
...
position<-Lag(ifelse(ma_20>ma_60, 1,-3))	#解說：position為一個時間序列，以日為單位，如果20ma大於60ma，設值為1；否則設值為0空手,-1做空。由於我們是日資料，訊號發生時只能隔天做交易，故將這向量全部往後遞延一天。
return<-ROC(Cl(h))*position	#ROC計算：log(今天收盤價/昨天收盤價)，乘上poistion代表。若1則持有，若0則空手
return<-return['2015-01-30/2016-03-21']	#由於我們策略條件是60ma>20ma之後才會交易，故統計值從2007-03-20開始；另外在有配發股利情況下容易失真
